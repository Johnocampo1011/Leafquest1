...
useEffect(() => {
  ...
  const increase = useCallback(async (type) => {
    ...
    const nextCooldownSeconds = careDefaults[`${type}Frequency`] * 86400; // days → seconds

    setCooldowns((prev) => ({
      ...prev,
      [type]: nextCooldownSeconds,
    }));
  ...
  const bar = (icon, value, type, label) => {
    ...
    const window = freqMs(type) * 0.2; // 20% tolerance window
    const endOnTime = recommended + window;
    const startOnTime = recommended - window;
...
const styles = StyleSheet.create({
  ...
  barsRow: { flexDirection: "row", justifyContent: "space-around", width: "100%", backgroundColor: "#fff", padding: 15,borderRadius: 12, elevation: 2 },
